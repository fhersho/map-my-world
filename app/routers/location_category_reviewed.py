...
from app.crud.location_category_reviewed import create_location_category_reviewed, get_location_category_reviewed, \
    get_location_category_reviews, get_recommendations
...
def create_location_category_reviewed(review: LocationCategoryReviewedCreate, db: Session = Depends(get_db)):
    return create_location_category_reviewed(db=db, review=review)
...
def read_location_category_reviewed(review_id: int, db: Session = Depends(get_db)):
    db_review = get_location_category_reviewed(db=db, review_id=review_id)
...
def read_location_category_reviews(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    return get_location_category_reviews(db=db, skip=skip, limit=limit)
